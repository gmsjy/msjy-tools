...
def nginx_log_cut():                    #因nginx日志太大，需要按500M分割，建立此函数进行分割日志
    if os.path.exists('/tmp/nginx_tmp/')!=True:       #分割日志的临时目录
       os.makedirs('/tmp/nginx_tmp/')
    if os.path.exists('/tmp/nginx_tmp_binfa/')!=True: #并发目录
       os.makedirs('/tmp/nginx_tmp_binfa/')
    if os.path.exists('/tmp/nginx_tmp_txt01/')!=True:  #time记录txt目录 
       os.makedirs('/tmp/nginx_tmp_txt01/')
    if os.path.exists('/tmp/nginx_tmp_txt02/')!=True:  #url记录txt目录 
       os.makedirs('/tmp/nginx_tmp_txt02/')
    if os.path.exists('/tmp/nginx_tmp_chuli/')!=True:  #处理所有txt目录
       os.makedirs('/tmp/nginx_tmp_chuli/')
    nginx_log_name=os.listdir('/tmp/nginx_log')[0].strip()  #切割日志名
    nginx_file='/tmp/nginx_log/%s' %nginx_log_name           #切割日志名及路径
    ...
    while True:                             #当条件为真执行，为假退出。
          ...
          file_name='/tmp/nginx_tmp/dd_access%d' %file_num  #分割成功的文件名
...
def nginx_log_time_count(file_name01):         #nginx分析日志函数
    file_name='/tmp/nginx_tmp_binfa/%s' %file_name01    #并发日志名
    file_txt='/tmp/nginx_tmp_txt01/%s.txt' %file_name01        #执行shell结果保存目录
    ...
    fuhao_cmd='%s' %fuhao
    f=open(file_txt)
    f1=open('/tmp/nginx_tmp_chuli/time_sum.txt','a')
...
def dxc_call_time_count():                       #多线程调用分析日志函数
    ...
    f=os.listdir('/tmp/nginx_tmp_binfa/')   #显示data/logs/nginx_tmp/目录下所有文件
...
def time_count_chuli():                         #time处理函数
    f=open('/tmp/nginx_tmp_chuli/time_sum.txt')
    time_max=[]
    for count in f:
...
def url_count_chuli():                          #url处理函数
    f=open('/tmp/nginx_tmp_chuli/url_sum.txt')
    url_max=[]
    for count in f:
...
def write_report_email():                       #写文件用来发email
    ...
    file=open('/tmp/nginx_log_email_tmp.txt','w')
    ...
    for line in url_count:
        ...
        file.write(fuhao_cmd)
    file.close()
    file=open('/tmp/nginx_log_email_tmp.txt','r')   #读取日志临时文件
    row=0
    file01=open('/tmp/nginx_log_email.txt','w')     #写文件
    for line in file.readlines():
        row=row+1                                         #row表示行数
    ...
    file.close()
    file01.close()
    os.remove('/tmp/nginx_log_email_tmp.txt')       #删除日志临时文件  
    os.remove('/tmp/nginx_tmp_chuli/time_sum.txt')       #删除time_sum文件 
    os.remove('/tmp/nginx_tmp_chuli/url_sum.txt')       #删url_sum文件  
def rmdir_nginx_log_mulu():       #清空日志目录函数
    shutil.rmtree('/tmp/nginx_tmp/')  #清空日志临时目录，供新日志存放
    os.mkdir('/tmp/nginx_tmp/')
    shutil.rmtree('/tmp/nginx_log/')  #清空日志目录，供新日志存放
    os.mkdir('/tmp/nginx_log')
def main():
   shutil.rmtree('/tmp/nginx_tmp_chuli')  #清空日志临时目录，供新日志存放
   os.mkdir('/tmp/nginx_tmp_chuli')
   cpu_he=count_cpu_heshu()                                       #cpu核数
   while len(os.listdir('/tmp/nginx_tmp/'))>0:   #动态统计分割日志文件个数
 f=os.listdir('/tmp/nginx_tmp/')          #动态统计分割日志文件
 key=0                                                       #默认key为0
 while (key<=cpu_he-1 and key<len(f)):      #对cpu核数进行对比
         name = '/tmp/nginx_tmp/%s' %f[key]    #日志文件名
         shutil.move(name,'/tmp/nginx_tmp_binfa/')  #移动日志文件，为了减少负载太高
         key=key+1
 dxc_call_time_count()
        dxc_call_url_count() 
 shutil.rmtree('/tmp/nginx_tmp_binfa/')
 os.mkdir('/tmp/nginx_tmp_binfa/')
        shutil.rmtree('/tmp/nginx_tmp_txt01/')
        os.mkdir('/tmp/nginx_tmp_txt01/')
        shutil.rmtree('/tmp/nginx_tmp_txt02/')
        os.mkdir('/tmp/nginx_tmp_txt02/')
   write_report_email()
